...
sys.path.append("/usr/local/lib/python2.7/dist-packages/scapy/")
...
def kbdecrypt(pkt, key = None, verbose = None):
    ...
    if pkt.getlayer(ZigbeeSecurityHeader).mic == '':
        ...
        mic = pkt.getlayer(ZigbeeSecurityHeader).mic
    ...
    #nonce += chr(fc | 0x05)
    crop_size =  len(encrypted) + len(mic)
    ...
    if verbose > 2:
        ...
        if micCheck == 1: print "\tMIC Check ok"

    frametype = pkt.getlayer(ZigbeeNWK).fields['frametype']
    if frametype == 0:
        # possibly wrong? -> only ZigbeeAppDataPayload
        if pkt.haslayer(ZigbeeAppDataPayload) and \
            pkt.getlayer(ZigbeeAppDataPayload).fields['aps_frametype'] == 1:
            payload = ZigbeeAppCommandPayload(payload)
        else:
